...
#The OG Window stuff but not really. This Opens when you press Mark Attendance when get_lect_deets returns false.

'''         Important Stuff.

            If you want to add Display Table and Display Graph after clicking Mark Attendance 
            for an unscheduled lecture, add it here. Just copy paste from below.
            BEFORE REMOVING THE self.string_path and self.sheet_name.
            I also added a Mark Attendance button here but didn't copy and paste the mark attendance function
            from the bottom. So it does nothing for now.

'''

# If mark attendance is clicked and unscheduled lecture was found then this window is created
...
class UIWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.btn_for_unscheduled.move(50, 120)
    ...
    def load_image_and_mark_attendance(self):
        ...
        lect_details, valid = get_lecture_details('Prakash Parmar')
        print(lect_details)
        ...
        try:
            ...
            if len(image_paths[0]) > 0:
                ...
                if len(matched_roll_numbers) > 0:
                    # date = today's date
                    mark_attendance(matched_ids=matched_roll_numbers, division=self.string_path,
    ...
    def table_disp(self):
        ...
        try:
            ...
            excel_to_display = pd.read_excel(path, header=None, index_col=0)
            os.remove(path)
            # print("Called model")
...
class UIToolTab(QWidget):
    ...
    def display_graph(self):
        try:
            plot_bargraph(lecture_type=self.sheet_name)
        except:
            choice = QMessageBox.question(self, 'Error!',
                                          "Incorrect path provided",
                                          QMessageBox.Ok)
        ...
        if len(image_paths[0]) > 0:
            ...
            if len(matched_roll_numbers) > 0:
                ...
                mark_attendance(matched_ids=matched_roll_numbers, division=lect_details['year_branch_div'],
                                lecture_type=lect_details['type'])
    ...
    def display_current_table(self, current_date):
        ...
        lecture_details, valid = get_lecture_details(professor_name)
...
class MainWindow(QMainWindow):
    # professor_name = 'Unknown'  # I'm sorry guys, I didn't know a better way to do this
    ...
    def startUIWindow(self):
        ...
        # If close session button is clicked then create the window with start session and faculty buttons
        self.Window.ToolsBTN.clicked.connect(self.startUIToolTab)

        self.show()

    #def home(self):   Ignore this for now
        '''
        self.log = QPushButton('LogOut', self)
        self.log.resize(250, 75)
        self.log.move(10, 10)
    ...
    '''def logger(self):
        ...
        self.login.setGeometry(100, 100, 605, 409)'''
